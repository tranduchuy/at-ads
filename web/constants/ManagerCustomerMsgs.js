module.exports = {
    ALREADY_MANAGED_BY_THIS_MANAGER: 'Hệ thống đã được quý khách chấp nhận quyền quản lý',
    ALREADY_INVITED_BY_THIS_MANAGER: 'Hệ thống đã gửi yêu cầu quyền quản lý đến tài khoản Google Ads của quý khách. Vui lòng kiểm tra lại.',
    ALREADY_MANAGED_IN_HIERARCHY: '',
    ALREADY_MANAGED_FOR_UI_ACCESS: '',
    NOT_AUTHORIZED: 'Không có quyền',
    ADD_CUSTOMER_FAILURE: '',
    CUSTOMER_NOT_ACTIVE: 'Tài khoản Google Ads chưa được kích hoạt',
    ACTION_NOT_PERMITTED: 'Không có quyền',
    TOO_HIGH: 'Quá lớn',
    TOO_LOW: 'Quá nhỏ',
    UNKNOWN: 'Không xác định được lỗi',
    INVALID_ID: 'Không tìm thấy criterionId',
    '': 'Không xác định'
  };